    ...
    return (
        <div className="product">
            <div className=''>
                ...
                <div className="cartBag position-relative d-flex justify-end flex-1">
            ...
            <ul className='nav nav-tabs'>
                    {
                        productTabs.map((eachLink, index) => (
                            <li className='nav-item' key={eachLink.id}>
                                <Link className={`nav-link text-white ${index === activeTab ? 'active' : ''}`} onClick={() => handlingTheActiveTabClick(index)} to={eachLink.routeName}>{eachLink.name}</Link>
                            </li>
                        ))
                    }
            </ul>
